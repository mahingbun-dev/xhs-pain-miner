...
**M0 已完成 ✅。M1① 与 M3② 已闭合 ✅；其余各门的报告与清单已产出，剩「你本人」的判断动作。**

🔶 = **代码已落地，验收门尚未闭合**。代码合入与验收通过是两件事 —— 这张表与下面的
路线图都不把前者当成后者，所以在验收完成前不标 ✅。

还差的门（材料都已备好）：M1② 人工盲评（报告与答题卡已产出，**需你逐张填**）·
M3① 最后一步点开 3 条证据链（真实链路 `collect`/`mine` 已跑通，**需你点链核对**）·
M2 竞品逐条抽检（清单已产出，**需你判断**）· M1③ 人工数 3 个痛点（原文读本已备）·
M1④ VLM 实测（**待多模态 Key**）。
代码审计已签署闭合（2026-09-19，见 [docs/code-audit.md](docs/code-audit.md)）。
真实采集基座为自研 [xhs-collector](../../xhs-collector)（ego-lite 浏览器），经 `plugin`
后端接入；上游 `xiaohongshu-mcp` 路线保留在 [docs/collector-mcp.md](docs/collector-mcp.md)。

**这些门差什么、从哪开始、卡在什么上** → [docs/acceptance-checklist.md](docs/acceptance-checklist.md)
的「下一步（TODO）」一节（判定标准与逐步做法也在同一页）。
