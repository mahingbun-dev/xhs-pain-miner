...
│   └─ MCPBackend       适配 xiaohongshu-mcp（Apache-2.0）       │
│                       适配器在仓库内，采集服务在用户本机        │
├──────────────────────────────────────────────────────────────┤
│                         基础设施                              │
...
| `collectors/` | 采集协议 + 样例后端 + 插件加载器 + **MCP 适配器** | ✅ M0（MCP 适配 M3） |
...
**唯一的例外是 MCP 适配器（M3），而划出这个例外的正是同一条规则 —— 许可证。**
上游 `xiaohongshu-mcp` 是 Apache-2.0，可商用、可再分发、无附加限制；被排除在外的是
「非商业学习许可」那一类。即便如此，适配器里也**没有一行采集代码**：`collectors/mcp.py`
只调用用户本机运行的那个服务的 HTTP 接口并做字段映射，采集本身仍在仓库之外。
安装、三个硬约束与故障排查见 [collector-mcp.md](collector-mcp.md)。

> 判据应当是"这份许可证允不允许我分发"，而不是"这份代码好不好用"。
> 后者会导致把许可受限的实现放进来，让整个项目从内部违反上游许可、污染下游权属。
...
> **尚未通过的验收门单独收在 [acceptance-checklist.md](acceptance-checklist.md)** ——
> 本表只回答"谁能做"，那份文件回答"怎么一步步做、做到什么算过"。
...
| **M2** | 竞品调研补全 | 竞品召回准确率达标，无误报 | 🔶 代码已合并，待人工抽检竞品调研准确率 |
| **M3** | 真实采集后端 | 真实后端跑通一个品类 + **代码审计：仓库不含受限代码** | 🔶 代码已合并，待真实后端跑通 + 代码审计 |
...
* **代码已合并进 `main`**（PR #3）。核心改动是竞品调研从「痛点名搜索」改为
  **解法词搜索**，并把结论从「有 / 没有」升级为四值状态（查到竞品 / 查证过确实
  没有 / 检索不到 / 调研失败）—— 设计理由见 4.5，实测数字见
  `tools/eval_research.py`。
* **验收门（竞品调研准确率人工抽检）尚未完成**。脚本只负责把待抽检的清单**摆到
  台面上**：`tools/eval_research.py --show-found` 会打印每条召回的竞品连同 URL 与
  平台描述，供逐条核对。与 M1 的盲评同理，这一步不能由 Agent 代劳。
* 脚本给出的"误报"只是**下界**：`expect_none` 只覆盖了能写成关键词的那一类
  （同名不同物），真正的准确率要靠上面这次抽检。

M3 当前状态说明：

* **代码已合并进 `main`**（PR #9）。适配器在 `collectors/mcp.py`，对接本机运行的
  `xiaohongshu-mcp`；走的是它的 REST 接口而非 MCP 协议端点（同一个服务、同一份数据），
  理由见该模块文档。
* **退出验收门两项都未完成**：① 用真实后端跑通一个品类 —— **需要你本机已登录的
  采集服务，不能由 Agent 代劳**；② 代码审计。
* 已知未做：**断点续跑**。中途失败需要重跑整轮。限速无需另做 —— 请求天然串行，
  且被对接的服务在每次导航/点击前后自带拟人延时。
* 三个硬约束（一次搜索只有一页、每篇笔记要再请求一次、评论按需走慢路径）写在
  [collector-mcp.md](collector-mcp.md)；它们会直接影响你能拿到多大的语料，
  而 M1 验收门里的"200 篇"这个后端单次达不到。
