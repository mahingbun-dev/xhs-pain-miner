...
class TestBuildCards:
    ...
    def test_thin_corpus_warning_copy_is_stable(self):
        ...
        ⚠️ 末尾那个 ``0.65`` 在本测试里**只是快照里的一个字面量**，它**不能**证明
        "提示里的数字与因子一致" —— 把 ``best`` 写死成 ``0.65``，这条照样通过。
        那个性质由下面 ``test_thin_corpus_warning_prints_the_number_instead_of_asserting``
        用 49 次这个取值钉住。

        早先这里写的是「头名不是满分」。改掉的理由**不是**"用户看到的是满分" ——
        报告里的因子条用两位小数（``render/*`` 的 ``:.2f``），49 次时显示的就是
        ``0.99``，那句话是**准确**的。真正的毛病是它**没告诉用户折扣有多大**：
        49 次只差 0.005、12 次差 0.35。印出数字，用户不必回头翻卡片就能判断这件事
        重不重要（PR #2 记的"已知残留 1"）。
        """
    ...
    def test_thin_corpus_warning_prints_the_number_instead_of_asserting(self):
        """★ 提示里印的必须是**算出来的**因子值，而不是一句"头名不是满分"。

        旧文案那句话是**准确的**（49 次时因子 0.995，报告里显示 0.99），毛病在于
        **信息量为零** —— 用户本来就看得见那个数，他看不出的是**折扣有多大**。
        现在把数字写进提示：49 次是 0.99、12 次是 0.65，"可忽略"与"该在意"一眼可辨。

        取 49 而不是 12 是关键：``0.995`` 是个**非整数**。写死任何单一常量都无法同时
        骗过这条与上面那条快照 —— 写死 ``0.65`` 能让快照通过但过不了这里；写死
        ``0.99`` 则反过来。50 次时不提示，因为那时确实是满分（见下面的边界测试）。
