...
"""警告文案里**每一类成因最多**列出几条检索词。

注意是**按类各自**的上限，不是整句话的上限：三类同时出现时点名数可以到 9 条
（独立验证实测过这一格 —— 别把它读成"整句最多 3 条"）。
    ...
    *,
    ...
    .. important::
       **这里只说"渠道上发生了什么"，不主张分数。**
       ...
       所以本函数里不再出现任何关于空白度取值的断言。「分数怎么算」这句话改由
       **确实知道最终结果的那几层**去说。判据是**它所在的位置能不能知道**，
       不是"它是哪个函数"—— 所以这份名单是**举例，不是穷举**：

       * :func:`~xhs_pain_miner.scoring.opportunity._unresolved_warning`（运行提示）
         —— 读合并之后的卡片；
       * 渲染层的卡片结论（``render/html.py`` 与 ``render/markdown.py`` 的
         "「竞品空白度」按中性值 0.5 计"）—— 同样读合并之后的卡片；
       * ``pain_miner`` 里几条"压根没查"的分支（调研关闭 / 超出
         ``RESEARCH_MAX_CLUSTERS``），以及
         :func:`~xhs_pain_miner.research.query.build_solution_queries` 的生成失败警告
         —— 这几个**自己构造**了那个 ``unsearchable`` 结论，不读卡片也知道结果。

       这几处说的都是真的（独立验证逐个实跑过）。违约的只是渠道层与轨迹层：
       它们手里只有一次请求，说不了一次运行。

       这条与本模块自己的立论同源：**主张只能由知道答案的那一层发出**。
       渠道层不知道合并结果，轨迹层不知道评分口径 —— 都不该替结论层下判断。

       .. admonition:: 写这类文字时踩过两次的坑

          不要用**绝对量词**。"只有 X 能说"、"它们**都**……"、"恒为 False" ——
          只要有一个反例，这句话本身就变成了一条新的失实文案。本分支上前后两次
          返工都出在这里（先是"只能由唯一知道最终结论的那一层说"，再是"它们都读的是
          合并之后的卡片"，反例恰恰是它自己列的第三条）。要么把反例穷举掉，要么
          写成"举例"。
    """
    if status == "ok":
...
def _unsearchable_warning(queries: Sequence[QueryTrace], *, subject: str) -> str:
    ...
    * **没查成**（限流 / 网络 / 平台接口变更 / 响应格式异常）—— "这次没跑完"。
      下一步要看具体原因：限流要等额度、接口变更要改代码。
    ...
    if missed:
        ...
        # ★ 不说"是限流或网络导致的"：那只是**常见**原因。渠道层还会因为响应不是合法
        # JSON、缺少 items/results 字段（平台接口变更）、非 429 的非 200 而失败 ——
        # 对那几类，"等额度或网络恢复"是错的建议（独立验证给的反例）。这里只说
        # "原因见轨迹"，把判断留给能看见 error 的用户。
        tips.append("没查成的那几条未必是同一类原因，具体见检索轨迹，排除后重跑即可")
