...
* 平台**返回过内容**但不相关 → ``no_competitor``（"查证过，确实没有"）
* 平台**压根没返回东西** → ``unsearchable``（"检索不到，无法判断"）

这两个类别**只描述该渠道上发生了什么**，不附带分数：空白度取决于**合并之后**的
结论，而本层看不到合并结果（早先这里写着"空白度 1.0 / 中性 0.5"，那个说法在
"本渠道检索不到、另一个渠道查到了竞品"时会变成假话 —— 与 M2 要消灭的自相矛盾
是同一类）。
    ...
    *,
    ...
    Raises:
        RuntimeError: 网络失败或响应格式异常。**这些都不是「没有竞品」** ——
            抛错代表**这条检索词没查成**，调用方不得把它读成"这个方向没有竞品"，
            否则一次网络抖动就会凭空造出一个高机会分的假机会（与 GitHub 渠道的
            ``_rate_limit_message`` 是同一条不变式）。

            注意这里**不**说"空白度按中性值处理"：那是**结论层**的判断。同渠道内
            若另有检索词已经查到了竞品，``classify_status`` 会给出 ``ok``、空白度
            按找到的竞品算 —— 渠道层不知道最终结论，说不了这件事。
    """
    if limit <= 0:
