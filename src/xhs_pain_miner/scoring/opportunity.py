...
def build_cards(
    ...
    *,
    ...
    if selected and max_size < MENTION_VOLUME_REFERENCE:
        ...
        #
        # 后者的毛病不是"不真实"，而是**信息量为零**：报告里的因子条本来就印着两位
        # 小数的因子值（`render/*` 用 ``:.2f``），49 次提及的头名显示为 0.99 ——
        # "不是满分"是用户自己就看得见的。他看不见的是**这个折扣有多大**：49 次时只差
        # 0.005（可忽略），12 次时差 0.35（该在意）。把数字写进提示，用户不必回头翻
        # 卡片就能判断这件事重不重要。
        #
        # 带上数字还有一个理由：卡片**未必**在报告里 —— Markdown 只渲染前
        # ``MAX_CARDS`` 张，排到后面的痛点根本没有因子条可看。提示自足才算数。
        # （PR #2 记的"已知残留 1"。）
